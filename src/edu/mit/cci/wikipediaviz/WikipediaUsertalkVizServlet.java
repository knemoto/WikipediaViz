	...
	throws IOException, ServletException {
		try {
			...
			if (pageTitle != null) {
				...
				GetRevisions gc = new GetRevisions();
				...
				if (articleCaches.isEmpty()) {
					...
					String download = gc.getArticleRevisions(lang,pageTitle,limit);
				...
				//log.info(data);
				lines = gc.sortMap(data,Integer.parseInt(nodeLimit));
				...
				if (edges.length() > 0) {
					...
					code = pro.processingCode(location, edges, loginUser, path, userName_editSize, size);
